...
# Use bash for process substitution etc.
...
def get_forward_primer(sample_id): return df.loc[sample_id]["Adapter_1"]
def get_reverse_primer(sample_id): return df.loc[sample_id]["Adapter_2"]
def get_year(sample_id):            return df.loc[sample_id]["Year"]
def get_date(sample_id):            return df.loc[sample_id]["Date"]

# ---------- alignment/index helpers ----------
def aligner():
    return config["params"].get("aligner", "bwa").lower()

def reference_fasta():
    fa = config["params"].get("reference_fasta", None)
    if not fa:
        raise ValueError("config.params.reference_fasta must be set (required by bcftools consensus/mpileup).")
    return fa

def prebuilt_index_prefix():
    return config["params"].get("index_prefix", None)

def chosen_prefix():
    # If a prebuilt index prefix is given, use it; else derive from reference_fasta basename
    if prebuilt_index_prefix():
        return prebuilt_index_prefix()
    fa = reference_fasta()
    return os.path.splitext(fa)[0]
...
def index_targets():
    if aligner() == "bowtie2":
        return bowtie2_index_files(chosen_prefix())
    else:
        return bwa_index_files(chosen_prefix())

# ---------- DAG ----------
rule all:
    input:
        # final per-sample consensus FASTA
        expand("{dir}/{sample_id}.masked_consensus.fasta", dir=config["dir_names"]["consensus_dir"], sample_id=sample_ids),
        # multiqc report
...
rule fastq_screen_pre:
    ...
    shell:
        ...
        # Create expected named targets if produced with slightly different naming
...
# -------------------- Index building (conditional) --------------------
rule build_bwa_index:
    input:
        fa = reference_fasta()
    output:
        lambda wc: bwa_index_files(chosen_prefix()) if aligner() == "bwa" else [temp("ignore_bwa_index")]
    params:
...
rule build_bowtie2_index:
    input:
        fa = reference_fasta()
    output:
        lambda wc: bowtie2_index_files(chosen_prefix()) if aligner() == "bowtie2" else [temp("ignore_bt2_index")]
    params:
        prefix = chosen_prefix()
    threads: 2
    run:
...
# Aggregator to ensure correct dependency on the right index set
rule ensure_index:
    input:
        lambda wc: index_targets()
    output:
...
rule pileup:
    ...
    shell:
        ...
        bcftools mpileup -A -Q {params.min_base_qual} --max-depth 5000000 -L 5000000 \
            -f {input.reference} {input.second_sorted_bam} > {output.pileup}
        """
...
rule call_snps:
    ...
    shell:
        ...
        bcftools mpileup -A -a "INFO/AD,INFO/ADF,INFO/ADR,FORMAT/ADF,FORMAT/ADR,FORMAT/SP" \
            -Q {params.min_base_qual} -f {input.reference} -L 5000000 --max-depth 5000000 \
...
rule index_bcf:
    ...
    resources: time_min=360, mem_mb=10000, cpus=1
    shell:
        "tabix -f {input.vcf} > {output.index}; tabix -f {input.consensus_vcf} > {output.consensus_index};"
...
rule mask_consensus:
    ...
    shell:
        r"""
        bcftools consensus -f {params.reference} -m {input.bed_file} {input.vcf} \
        | sed 's/>MN908947\.3/>hCov-19\/USA\/NY-{params.sample}\/{params.year}/' > {output.consensus_genome}
        """
